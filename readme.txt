...
3. To use the GUI, make sure JavaFX is installed and enabled in your Intellij to use the GUI. (JavaFX comes standard with JDK 8)
...
send a maximum of 5 requests per minute so sometimes it will only return certain info for a share. If we were able to buy the premium account for the API, we could request up to a lot more.
...
== KNOWN BUGS FOUND AFTER DEADLINE ==
We have found some bugs in our program after the code deadline
- When depositing into a stock account, due to converting errors you get 30% more money than you deposit, transfers into stock accounts work fine though.
- When depositing into a debt account, you may get an email saying you have cleared the max debt and your account has been reactivated even though you never passed max debt.
